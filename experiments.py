    ...
    class chairs_model():
        def __init__(self):
            ...
            self.ph_sm_bdw_occ = pretrain_models.abs_mp(path='./pretrain_models/chairs/irrpwc_ph_sm_bdw_occ_EPE=3.07.pth',
                                                        ...
                                                        note='single forward flow, photo loss, smooth loss and occ check, augmentation is closed, lr=1e-5')
            self.ph_sm_bdw_occ_eq = pretrain_models.abs_mp(path='./pretrain_models/chairs/irrpwc_ph_sm_bdw_occ_eq_EPE=2.89.pth',
                                                           info='flyingchairs(EPE=3.07 )',
                                                           note='single forward flow, only photo loss and smooth loss, augmentation is used, lr=1e-5')
...
def flying_chairs_experiment():
    ...
    # the third stage: add self-supervision(I call Equivariant loss or eq loss here) EPE=2.89
    elif stage == 3:
        param = {
            'if_show_training': False,
            'train_dir': '/data/Optical_Flow_all/train_OIFlow_irrpwc_chairs_3',
            # ==== dataset
            'train_set': 'flyingchairs',
            'validation_sets': ('flyingchairs',),
            'eval_batch_size': 8,
            ...
            # ==== model
            'pretrain_path': pretrain_models.chairs_model().ph_sm_bdw_occ.path,  # using model from stage 1
            'if_froze_pwc': False,
            'if_use_correlation_pytorch': False,  # just for debug
            ...
            # ==== loss
            'train_func': 'train_bi_direction_photo_occ_smooth_eqloss',
            'photo_loss_weight': 1,
            'norm_photo_loss_scale': 0.5,  # the original image scale is 0~255
            'photo_loss_type': 'abs_robust',  # abs_robust, charbonnier,L1,SSIM(can not use, only for occlusion checking is enabled)
            'smooth_loss_type': 'delta',  # 'edge' or 'delta'
            ...
            'equivariant_hflip': True,
            'equivariant_rotate': (-0.2, 0.2, -0.015, 0.015),  # (-0.01, 0.01, -0.01, 0.01)
            'equivariant_squeeze': (0.86, 1.16, 0.86, 1.16),  # (1.0, 1.0, 1.0, 1.0)
            'equivariant_trans': (0.4, 0.4),  # (0.04, 0.005)
            'equivariant_vflip': False,
            'equivariant_zoom': (1.0, 1.5, 0.985, 1.015),  # (1.0, 1.4, 0.99, 1.01)
        }
    # the forth stage: add occlusion inpaining todo now training
    elif stage == 4:
